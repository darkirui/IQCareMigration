...
, CASE WHEN OI LIKE '%pulmonary%tb%' THEN 'Pulmonary TB' ELSE a.OI END AS OI
FROM (
...
, c.*
*/
a.OI, COUNT(a.Ptn_Pk)n

 FROM OIs a 
	left join LookupItemView c on a.OI = c.DisplayName and c.MasterName LIKE 'WHOStage%Conditions'
WHERE c.ItemDisplayName is null
GROUP BY a.OI 
ORDER BY n
*/